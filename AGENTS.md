...
- **`StockDBProvider`**: StockDB 本地 LevelDB 时序引擎驱动（127.0.0.1:7899），支持 1m 超高频与 1d 全截面多因子、A 股个股与退市股全覆盖、场内 ETF 独立覆盖、同花顺概念与申万行业平铺映射。内置 `stockdb.pyd` 与 TCP Socket 快速探针。
  - **1m 高频单次大跨度查询源头截断铁律 (服务端内伤)**：StockDB 单次大跨度查询（如跨 2 年 ~10 万根）会因底层 LevelDB 迭代器跨 SSTable 游标上限与 Socket 发送缓冲区溢出而发生静默截断（如实测 `sh.510050` 单次拉取必丢 7,943 根，仅返回 89,874 根）；**重置连接对单次大查询完全无效**（因服务端在源头直接掐断传输并提前发送 EOF），必须严格采用自然月分段拉取（Monthly Chunking，单月 ~5,000 根），确保数据 100% 完整无损。
  - **标的间 Socket 物理排空重置规范 (传输层外伤)**：单长连接 TCP Socket 在无间隔超高频循环调用（如 16 分钟内 80,000+ 次查询）时，因微小时延累积会导致操作系统接收缓冲区滞后粘包错位（多米诺骨牌效应：前数十只标的误报 0，后一只标的暴增吐出 18 万行）；必须在每次进入 1m 切片前由驱动层统一调用 `reset_connection()`（耗时 <0.8ms），由 OS 内核直接销毁旧连接残留缓冲区；**严禁隐式防抖重试与假容错**，纯粹依靠物理连接复位确保数据一次性绝对真实一致。
  - **严禁多线程与 Async 访问 StockDB 禁令 (并发红线)**：
    - 实测证据：对 8 只主流 ETF 压测证明，8 线程独立连接并发会导致服务端 LevelDB 游标多路竞争，**数据损毁率高达 87.5%**（`sh.588000` 丢 1.6 万行，`sz.159949` 丢 1 万行）；Async 8 路并发导致时序彻底颠倒，**损毁率 100%**；且并发因受限于单引擎磁盘扫描与 C 扩展 Mutex，耗时（~14.5s）**完全没有任何提速**。
    - 运行规范：对 StockDB 的所有访问**必须严格保持单连接串行流式**，坚决杜绝在外部引入 ThreadPool/ProcessPool 或协程并发。
...
- **外部数据源 SDK 与二进制库原则**:
  - 纳入工程的第三方/外部驱动文件（如 `cq/data/provider/stockdb/` 下的 `stock_sdk.py`, `zhibiao.py`, `stockdb.pyd` 等）属于外部库原生组件，**严禁私自修改其内部源码**；
  - 必须严格遵从其**官方公开 API** 进行接入与调用，**严禁使用内部未公开私有属性/方法或通过修改第三方源码进行伪适配**；
  - 适配层的职责仅限于在 `BaseProvider` 实现类（如 `StockDBProvider`）内部做好按需安全加载、参数标准化、时区清洗与数据类型规约。
